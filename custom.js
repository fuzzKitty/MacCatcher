	...
	var vs = new ViewState(view_str);
      if(vs.isValid) {
		...
		if(page_content.includes(generator_reg)){
			...
			var generator_str_hex = generator_str[6] + generator_str[7] + generator_str[4] + generator_str[5] + generator_str[2] + generator_str[3] + generator_str[0] + generator_str[1];
				...
				for (let i = 0; i < val_keys.length; i++) {
					if(val_keys[i].split(",").length == 2){
						...
						try{
							...
							hmacObj.setHMACKey(val_key, "HEX");
							hmacObj.update(ascii_to_hex(data_to_hash));
							var computed_hash = hmacObj.getHMAC("HEX").toUpperCase();

							if(computed_hash == ascii_to_hex(enc_data.substr(enc_data.length - hashSize))){
								alert("Awwwwwwww Snap!!!!!!\nDecription Key: " + dec_key + "\nValidation Key: " + val_key);
								console.log("Awwwwwwww Snap!!!!!!\nDecription Key: " + dec_key + "\nValidation Key: " + val_key);
							}
						}
